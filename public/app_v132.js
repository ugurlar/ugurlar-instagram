...
async function loadShopifyStatus(products) {
  for (const product of products) {
    ...
    try {
      // Robust fetch with simple retry
      let data = null;
      let retries = 2;
      ...
      while (retries >= 0) {
        try {
          ...
          if (data.found || response.ok) break;
        } catch (e) {
          if (retries === 0) throw e;
          console.warn(`Retrying shopify fetch for ${code}...`);
      ...
      if (data && data.found && data.url) {
        ...
        badgeContainer.innerHTML = `
          <div class="badge" style="border-color: #22c55e; color: #22c55e; background: rgba(34, 197, 94, 0.1); font-weight: 600; display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px;">
